...
import Jabrooni from "./pages/Jabroni";
...
function App() {
  ...
  async function mint(mintCount) {
    if (contract) {
      if (chainId === 1) {
        if (mintCount === 0) {
          swal("Minimum mint amount is 1 Jabrooni", "", "info");
        } else {
          try {
            const finalPrice = Number(price) * mintCount;
            await contract.methods
              .mintJabroniMoonHouse(mintCount)
        ...
        swal("Please switch to mainnet to mint Jabrooni", "", "error");
      }
    } else {
      swal(
        "",
